...
// Lightweight CJS-compatible faker replacement to avoid ESM-only @faker-js/faker
const faker = (() => {
  // Built-in UUID v4 generator — avoids ESM-only uuid package in Bruno's CJS sandbox
  const _uuidv4 = () => {
    const hex = '0123456789abcdef';
    const seg = (n) => Array.from({ length: n }, () => hex[Math.random() * 16 | 0]).join('');
    return `${seg(8)}-${seg(4)}-4${seg(3)}-${hex[8 + (Math.random() * 4 | 0)]}${seg(3)}-${seg(12)}`;
  };
  const rand = (min, max) => {
  ...
  return {
    seed: () => { /* no-op for CJS faker replacement */ },
    setLocale: undefined,
    locale: undefined,
    helpers: {
      arrayElement: (arr) => arr[rand(0, arr.length - 1)],
    },
    string: {
      uuid: () => _uuidv4(),
    },
    internet: {
      email: () => `${firstNames[rand(0, firstNames.length - 1)].toLowerCase()}${rand(1,999)}@${domains[rand(0, domains.length - 1)]}`,
      url: () => `https://${domains[rand(0, domains.length - 1)]}/${words[rand(0, words.length - 1)]}`,
    },
    date: {
      past: () => new Date(Date.now() - rand(1, 365) * 86400000),
    },
    person: {
      fullName: () => `${firstNames[rand(0, firstNames.length - 1)]} ${lastNames[rand(0, lastNames.length - 1)]}`,
    },
    lorem: {
      sentence: () => words.slice(0, rand(3, 8)).join(' ') + '.',
    },
    number: {
      int: ({ min = 0, max = 1000 } = {}) => rand(min, max),
      float: ({ min = 0, max = 1000 } = {}) => min + Math.random() * (max - min),
    },
    datatype: {
...
class SchemaValidator {
   ...
   * @param {*} json - Sample JSON data
   * @returns {object} Generated and enriched JSON schema
   * @private
   */
   ...
   * @param {*} body - The validated data
   * @param {string} folderName - Schema folder name
   * @param {string} fileName - Schema file name
   * @param {string} schemaFilePath - Full path to the schema file
   * @private
   */
   ...
   * @param {string|object} folderNameOrSchema - Folder name or inline schema object
   * @param {string|*} fileNameOrData - File name or data to validate (inline mode)
   * @param {object} [body] - The object to validate (file-based mode)
   ...
   * @param {boolean} [options.throwOnError] - Throw error instead of returning false
   * @returns {boolean|object} The result of the validation
   */
  validateJsonSchemaSync(folderNameOrSchema, fileNameOrData, body, options = {}) {
    ...
    if (!validRes) {
      if (verbose) {
        this._logValidationErrors(validate.errors, body, folderName, fileName, schemaFilePath);
      }
    } else {
      if (verbose) {
   ...
   * Supports two modes:
   * - Inline: trackSchemaVersion(name, schema)
   * - File-based: trackSchemaVersion(folderName, fileName, version)
   * @param {string} nameOrFolder - Schema name or folder name
   * @param {object|string} schemaOrFile - Schema object (inline) or file name (file-based)
   * @param {string} [version] - Semantic version string (file-based mode only)
   */
   ...
   * Supports two calling modes:
   * - snapshot(name, data, ignoreFields)
   * - snapshot(category, name, data, options)
   * @param {string} nameOrCategory - Snapshot name or category
   * @param {*} dataOrName - Data to snapshot or sub-name (category mode)
   * @param {*} [ignoreFieldsOrData] - Ignore fields array or data (category mode)
   * @param {object} [optionsArg] - Options (category mode)
   * @returns {object} Snapshot result with created/matched/path
   */
  snapshot(nameOrCategory, dataOrName, ignoreFieldsOrData, optionsArg) {
    ...
    const snapshotDir = path.join(this.schemaBasePath, '__snapshots__');
   ...
   * Supports two calling modes:
   * - validateSnapshot(name, data, ignoreFields)
   * - validateSnapshot(category, name, data, options)
   * @param {string} nameOrCategory - Snapshot name or category
   * @param {*} dataOrName - Data or sub-name (category mode)
   * @param {*} [ignoreFieldsOrData] - Ignore fields array or data (category mode)
   * @param {object} [optionsArg] - Options (category mode)
   ...
   * Supports two modes:
   * - Inline: registerEnvironmentSchema(name, env, schema)
   * - File-based: registerEnvironmentSchema(env, folderName, fileName, schemaPath)
   * @param {string} nameOrEnv - Schema name or environment name
   * @param {string} envOrFolder - Environment or folder name
   * @param {object|string} schemaOrFileName - Schema object or file name
   * @param {string} [schemaPathArg] - Path to schema file (file-based mode)
  ...
  registerEnvironmentSchema(nameOrEnv, envOrFolder, schemaOrFileName, schemaPathArg) {
    ...
    this._envSchemas[key] = JSON.parse(fs.readFileSync(schemaPath, 'utf8'));
    return true;
  }
   ...
   * Supports two modes:
   * - Inline: generateDocumentation(title, schema, options)
   * - File-based: generateDocumentation(folderName, outputDir)
   * @param {string} titleOrFolder - Document title or folder name
   * @param {object|string} schemaOrOutputDir - Schema object or output directory path
   * @param {object} [options] - Options (inline mode: endpoint, method, examples)
   * @returns {string} Generated markdown content or path to file
   */
...
<body>
   ...
   * Supports two modes:
   * - Inline: validateWithTolerance(schema, data, toleranceOptions)
   * - File-based: validateWithTolerance(folderName, fileName, body, toleranceOptions)
   * @param {object|string} schemaOrFolder - Schema object or folder name
   * @param {*} bodyOrFile - Data or file name
   * @param {*} toleranceOrBody - Tolerance options (inline) or body (file-based)
   * @param {object} [toleranceOptions] - Tolerance settings (file-based mode)
   ...
   * Supports two modes:
   * - Inline: modifySchema(schema, modificationsObject)
   * - File-based: modifySchema(folderName, fileName, modifications)
   * @param {object|string} schemaOrFolder - Schema object or folder name
   * @param {*} modsOrFile - Modifications object/array or file name
   * @param {Array} [modifications] - Modification operations (file-based mode)
   ...
   * Supports two modes:
   * - Inline (sync): benchmarkValidation(schema, data, options)
   * - File-based (async): benchmarkValidation(folderName, fileName, sampleData, options)
   * @param {object|string} schemaOrFolder - Schema object or folder name
   * @param {*} dataOrFile - Data or file name
   * @param {*} [optionsOrData] - Options (inline) or sample data (file-based)
   * @param {object} [optionsArg] - Options (file-based mode)
   * @returns {object|Promise<object>} Benchmark results
   */
  benchmarkValidation(schemaOrFolder, dataOrFile, optionsOrData, optionsArg) {
    ...
    if (typeof schemaOrFolder === 'object' && schemaOrFolder !== null) {
      ...
      return this._computeBenchmarkStats(times, iterations);
    }
...
// These are designed for quick use inside Bruno CLI test scripts where you
// don't want to instantiate SchemaValidator just to do a one-off check.
...
/**
 * Assert that data matches a schema — throws on failure (for Bruno tests / assert blocks).
 * @param {object} schema - JSON Schema object
 * @param {*} data - Data to validate
 ...
 * Works with Bruno's `res` object or any object with a `status`/`statusCode` property.
...
/**
 * Assert that the response body contains specific fields (shallow or deep via dot-notation).
 * @param {object} body - Response body
 * @param {string[]} fields - Array of field names or dot-paths (e.g. ['id', 'user.name'])
 */
...
/**
 * Quick schema builder — creates a JSON Schema object from a sample value.
 * Useful in Bruno tests to auto-generate a schema for contract testing.
...
/**
 * Check if data contains potential PII (quick standalone check).
