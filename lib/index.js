...
/**
 * Register all custom formats on an AJV instance.
 * Replaces the deprecated `ajv-formats` package.
 * @param {object} ajv - AJV instance
 * @param {object} [extraFormats] - Additional format definitions to register
 */
function _addFormats(ajv, extraFormats = {}) {
  const allFormats = { ...customFormats, ...extraFormats };
  for (const [name, pattern] of Object.entries(allFormats)) {
    ajv.addFormat(name, pattern instanceof RegExp ? { type: 'string', validate: (s) => pattern.test(s) } : pattern);
  }
}

/**
 * Infer a JSON Schema from a sample value (replaces the abandoned `generate-schema` package).
 * Handles objects, arrays, and primitives recursively.
 * @param {*} value - Sample data
...
function _inferSchema(value) {
  ...
  if (Array.isArray(value)) {
    ...
    if (value.length > 0) {
      // Use the first non-null item to infer items schema
      const sample = value.find(v => v !== null && v !== undefined);
      if (sample !== undefined) schema.items = _inferSchema(sample);
    }
    return schema;
  }
  ...
  if (typeof value === 'number') return Number.isInteger(value) ? { type: 'number' } : { type: 'number' };
...
class SchemaValidator {
  ...
  _generateSchemaFromData(json) {
    const schema = _inferSchema(json);
  ...
  validateJsonSchemaSync(folderNameOrSchema, fileNameOrData, body, options = {}) {
    ...
    if (typeof folderNameOrSchema === 'object' && folderNameOrSchema !== null) {
      ...
      _addFormats(ajv, this.options && this.options.customFormats);
...
function schemaFrom(sample) {
  const schema = _inferSchema(sample);
  schema['$schema'] = 'http://json-schema.org/draft-07/schema#';
  return schema;
}
