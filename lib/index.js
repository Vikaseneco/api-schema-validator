...
class SchemaValidator {
   ...
   * @param {string} schemaBasePath - Base directory for schema files (optional, defaults to './api-schemas')
   *                                  Can be relative (resolved from process.cwd()) or absolute path
   */
  constructor(schemaBasePath = './api-schemas') {
    // If path is relative, resolve it from current working directory
    // This ensures it works correctly in Bruno which runs from its own directory
    if (!path.isAbsolute(schemaBasePath)) {
      this.schemaBasePath = path.resolve(process.cwd(), schemaBasePath);
    } else {
  ...
  /**
   * Create a SchemaValidator instance with Bruno collection path
   * This is a convenience method for Bruno API testing
   * @param {object} bru - The Bruno context object (available in .bru test files)
   * @param {string} schemaFolder - Name of the schema folder (default: 'api-schemas')
   * @returns {SchemaValidator} A new SchemaValidator instance
   * @example
   * const validator = SchemaValidator.forBruno(bru, 'api-schemas');
   */
  static forBruno(bru, schemaFolder = 'api-schemas') {
    if (!bru || typeof bru.cwd !== 'function') {
      throw new Error('Invalid Bruno context object. Make sure to pass the "bru" object from your .bru test file.');
    }
    const collectionPath = bru.cwd();
    const schemaPath = path.join(collectionPath, schemaFolder);
