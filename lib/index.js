...
class SchemaValidator {
   ...
   */
  trackSchemaVersion(folderName, fileName, version) {
   ...
   */
  registerEnvironmentSchema(env, folderName, fileName, schemaPath) {
    if (!this._envSchemas) {
      this._envSchemas = {};
    }
   ...
   */
  generateDocumentation(folderName, outputDir = './docs') {
...
<body>
  ...
  migrateSchema(schema, transformations = []) {
    ...
    for (const transform of transformations) {
      switch (transform.type) {
        case 'rename_field':
          if (migrated.properties && migrated.properties[transform.oldName]) {
            migrated.properties[transform.newName] = migrated.properties[transform.oldName];
            delete migrated.properties[transform.oldName];
            
            if (migrated.required) {
              const idx = migrated.required.indexOf(transform.oldName);
              if (idx !== -1) {
                migrated.required[idx] = transform.newName;
              }
            }
          }
          break;
   ...
   */
  validateWithTolerance(folderName, fileName, body, toleranceOptions = {}) {
    const schemaFilePath = path.join(this.schemaBasePath, folderName, `${fileName}_schema.json`);
  ...
  modifySchema(folderName, fileName, modifications = []) {
    const schema = this.getSchema(folderName, fileName);
    if (!schema) {
      throw new Error(`Schema not found: ${folderName}/${fileName}`);
    }
    
    const modified = JSON.parse(JSON.stringify(schema)); // Deep clone
   ...
   */
  async benchmarkValidation(folderName, fileName, sampleData, options = {}) {
